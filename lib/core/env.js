 ...
 * @Last Modified time: 2018-07-18 18:58:04
 * 读取配置文件
...
function get () {
    ...
    if (process && process.argv) {
        process.argv.forEach(
            (param, i) => {
                ...
                if (param.indexOf('--scope.') > -1 && process.argv[i + 1] && process.argv[i + 1].indexOf('-') !== 0){
                    proscope[param.substring('--scope.'.length)] = process.argv[i + 1];
                } else if (param.indexOf('--scope.') > -1) {
                    proscope[param.substring('--scope.'.length)] = true;
                }
            }
        );
    }
    ...
    ['dict'].forEach(key => {
        let fp = proscope[key];
        if (fp) {
            proscope[key] = path.resolve(process.cwd(), fp);
        }
    });
    ...
    fileList.forEach(
        (filePath) => {
            if (endWith(filePath, 'json') || endWith(filePath, 'conf')) {
        ...
        function scopeArray(config, scope) {
            ...
            config.forEach((item, index) => {
                if (Array.isArray(item)) {
                    ...
                    _scope =  Object.assign(_scope, item.scope, proscope);
        ...
        function scopeObject(config, scope) {
            ...
            Object.keys(config).forEach(key => {
                ...
                if (Array.isArray(target)) {
                    ...
                    _scope =  Object.assign(_scope, target.scope, proscope);
...
function redirectPathConfig(config, deep) {
    ...
    function dealString(target) {
        if (target.indexOf('config/') === 0) {
            ...
            arr.shift(0);
            let t = getConfigByAttrs.apply(this, arr);
            if (t && typeof t === "object") {
                target = redirectPathConfig(t, deep);
            } else {
                target = path.resolve(env.root, target);
            }
        }
        return target;
    }
...
function endWith (path, mark) {
    if (path.indexOf(mark) === (path.length -  mark.length)) {
        return true;
    }
    return false;
}