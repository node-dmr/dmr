...
class ExtractTransform extends Transform {
    ...
    check(condition, value) {
        let rs = true;
        ["_gt", "_eq", "_lt", "_gt_eq", "_lt_eq", "_match", "_not_match", "_not_eq", "_not_in_dict", "_in_dict"].forEach(method => {
    ...
    _in_dict(condition, value) {
        let rs = true;
        if (condition["in-dict"] && condition.dict && condition.dict.loaded) {
            rs = false;
            if (condition.decodeURIComponent) value = encodeURIComponent(value);
            condition.dict.each(item => {
                if (condition.encodeURIComponent) item = encodeURIComponent(item);
                if (item == value) {
                    rs = true;
                }
            });
    ...
    _not_in_dict(condition, value) {
        let rs = true;
        if (condition["not-in-dict"] && condition.dict && condition.dict.loaded) {
            if (condition.decodeURIComponent) value = encodeURIComponent(value);
            condition.dict.each(item => {
                if (condition.encodeURIComponent) item = encodeURIComponent(item);
                if (item === value) {
    ...
    _normalizeCondition(conditions) {
        conditions.forEach(condition => {
            ...
            if (condition["not-in-dict"] || condition["in-dict"]) {
                condition.dict = new Dict({
                    filePath: condition["not-in-dict"] || condition["in-dict"]
                });
            }
        });
