 ...
 * @Last Modified time: 2018-05-12 15:41:52
 */
var fs  = require('fs');
...
class SuperTask extends Task{
    ...
    run (action) {
        ...
        let outputInterval = this.config["interval"] || this.config["output-interval"];
        let inputInterval = this.config["interval"] || this.config["input-interval"];
        ...
        outputRanges.forEach((range, index) => {
            this.actions.push(Object.assign({}, action, {range: range.param()}));
        });
        this.next();
    }
