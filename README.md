...
Alpha(0.3.x) version will be released  on NPM before **July 30th**.

It`s not ready for public before Alpha version.

I'm trying to write Cli and Add Docs / Language support / Unit Test.

Thank you for your attention!

## 简介
DMR 是一个开放过程的数据处理框架，除了提高业务数据报表产出的效率外，更注重让更多业务角色快速、独立参与数据分析。框架主要聚焦数据处理过程（入库、转储、Map、Reduce、数据分析、数据评价、报表整合、任务调度、日程管理等）。

DMR 主要处理“原数据”至“报表数据”产出过程，不负责数据存储及数据通路，不负责数据展现。但是框架会兼容绝大多数的数据接入方式（如Ftp、Hadoop、北斗），及数据报表格式（ShowX、Spy、Speedup等），无缝关联指标及各展现平台结果。
...
在本章节我们的目标安装框架，使用框架的bin来完成初始化，并根据demo配置完成一个数据分析过程，包括入库（import）/转储（transfer）/计算处理（caculate）等。最终以可视化形式展现分析结果。
...
DMR是一个工具框架，要与配置结合才能正常运行起来。配置在项目（Project）工程下的\config目录下，包含了所需的各种配置
