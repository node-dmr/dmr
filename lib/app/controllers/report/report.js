 ...
 * @Last Modified time: 2018-06-27 12:22:35
 */
const fs = require('fs');
...
class ReportController extends Controller{
    ...
    getChainDims(req, res, next) {
        ...
        let range = new Range(JSON.parse(rangeString));
        let rej = new ResponseJson(), data = [];
        if (taskid) {
            let config = Config.find('task',taskid)["output-source"];
            let source = new SourceFile(config);
            ...
            parser.on('end', () => {
                ...
                Object.keys(dims).forEach(dim => {
                    dimParten.push(JSON.parse(dim));
                });
                let chain = new Chain(dimParten, dconditions.keys());
                rej.set('data', chain.get());
                res.send(rej.get());
            });
